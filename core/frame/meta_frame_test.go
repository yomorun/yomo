...
func TestMetaFrameEncode(t *testing.T) {
	...
	m.SetDispatch(DispatchBroadcast)
	result = append(result, byte(TagOfDispatch), 0x1, 0x1)
	assert.Equal(t, result, m.Encode())
}

func TestMetaFrameDecode(t *testing.T) {
	buf := []byte{0x80 | byte(TagOfMetaFrame), 0x0C, byte(TagOfTransactionID), 0x04, 0x31, 0x32, 0x33, 0x34, byte(TagOfSourceID), 0x01, 0x31, byte(TagOfDispatch), 0x01, 0x01}
	meta, err := DecodeToMetaFrame(buf)
	assert.NoError(t, err)
	assert.EqualValues(t, "1234", meta.TransactionID())
	assert.EqualValues(t, "1", meta.SourceID())
	assert.EqualValues(t, DispatchBroadcast, meta.Dispatch())
	t.Logf("%# x", buf)
}
