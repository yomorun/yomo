...
Followed the [tutorial in `yomo run`](/docs/cli/run), let's implement ourselves application to generate the data. assume the [Zipper Service][zipper] is already listening on `localhost:9000`:
