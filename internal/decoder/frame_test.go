...
func TestReadBytesFromFrameDecoder(t *testing.T) {
	t.Run("Read raw bytes from frame decoder", func(t *testing.T) {
		b := &bytes.Buffer{}
		// first 3 bytes indicates the length of frame is 6.
		// the bytes are full matched.
		b.Write([]byte{0, 0, 6, 0, 0, 0, 1, 2, 3})
		...
		b.Reset()
		// more bytes, only read first 6 bytes.
		b.Write([]byte{0, 0, 6, 0, 0, 0, 1, 2, 3, 4, 5})
		decoder = NewFrameDecoder(b)
		actual, err = decoder.Read(true)
		expected = []byte{0, 0, 0, 1, 2, 3}
		assert.NoError(t, err)
		...
		b.Write([]byte{0, 0, 8, 0, 0, 0, 1, 2, 3})
		b.Write([]byte{4, 5})
		decoder = NewFrameDecoder(b)
		actual, err = decoder.Read(true)
	...
	t.Run("Read frame bytes from frame decoder", func(t *testing.T) {
		b := &bytes.Buffer{}
		// first 3 bytes indicates the length of frame is 6.
		b.Write([]byte{0, 0, 6, 0, 0, 0, 1, 2, 3, 5, 6})

		decoder := NewFrameDecoder(b)
		actual, err := decoder.Read(false)
		expected := []byte{0, 0, 6, 0, 0, 0, 1, 2, 3}
		assert.NoError(t, err)
		assert.Equal(t, expected, actual)
	})
}
...
func TestReadBrokenFromFrameDecoder(t *testing.T) {
	...
	// first 3 bytes indicates the length of frame is 8.
