...
func TestReadRawBytesFromDecoder(t *testing.T) {
	...
	b.Write([]byte{0, 0, 3, 1, 2, 3})

	// mock observable
	observable := FromStream(b)
	// get raw bytes
...
func TestObserveDataFromY3Decoder(t *testing.T) {
	t.Run("Observe data from Y3 Node Packet", func(t *testing.T) {
		b := &bytes.Buffer{}
		// first 3 bytes indicates the FrameLength, the rest are the raw bytes which encoded by Y3 Codec.
		b.Write([]byte{0, 0, 29, 129, 3, 144, 25, 17, 4, 65, 233, 15, 156, 18, 6, 175, 170, 192, 218, 156, 100, 19, 9, 108, 111, 99, 97, 108, 104, 111, 115, 116})
	...
	t.Run("Observe data by multi keys", func(t *testing.T) {
		b := &bytes.Buffer{}
		// first 3 bytes indicates the FrameLength, the rest are the raw bytes which encoded by Y3 Codec.
		b.Write([]byte{0, 0, 9, 16, 3, 1, 2, 3, 17, 2, 1, 2})
