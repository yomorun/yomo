...
type StreamFunction interface {
	...
	Write(tag Tag, carriage []byte) error
}
...
type streamFunction struct {
	...
	zipperAddr      string
	client          *core.Client
	observeDataTags []Tag             // tag list that will be observed
...
// SetObserveDataTags set the data tag list that will be observed.
// Deprecated: use yomo.WithObserveDataTags instead
func (s *streamFunction) SetObserveDataTags(tag ...Tag) {
...
func (s *streamFunction) Write(tag Tag, carriage []byte) error {
	frame := frame.NewDataFrame()
